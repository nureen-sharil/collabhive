...
// ─── date helpers (same as CreateMeeting) ────────────────────────────────────
const now        = new Date();
...
export function EditTask() {
  ...
  // Time state
  const [timeSet,     setTimeSet]     = useState(!!task?.dueTime);
    ...
    return (
      <PhoneFrame>
        <div style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center" }}>
          <p style={{ color: "#9CA3AF" }}>Task not found.</p>
        </div>
      </PhoneFrame>
    );
  }
    ...
    <PhoneFrame indicatorBg="#ffffff">
      ...
      <div className="bg-white flex-shrink-0 flex items-center justify-between px-5 py-3"
        ...
        <div className="flex items-center gap-3">
          ...
          <span style={{ fontSize: 16, fontWeight: 600, color: "#111827" }}>Edit Task</span>
        </div>
        {/* Current status badge */}
        <span style={{ fontSize: 12, fontWeight: 600, background: "#F3F4F6", color: "#374151", borderRadius: 20, padding: "4px 10px" }}>
          {statusInfo.label}
        </span>
      </div>
        ...
        <div className="space-y-4">
          ...
          <div className="bg-white rounded-2xl p-4 shadow-sm">
            <div className="flex items-center gap-1.5 mb-2">
              <AlignLeft size={13} color="#374151" />
              <span style={{ fontSize: 13, fontWeight: 600, color: "#374151" }}>Task Title <span style={{ color: "#EF4444" }}>*</span></span>
            </div>
            <input type="text" value={title} onChange={(e) => setTitle(e.target.value)} style={inputBase} placeholder="Task title" />
          </div>
          ...
          <div className="bg-white rounded-2xl p-4 shadow-sm">
            ...
            <textarea value={description} onChange={(e) => setDescription(e.target.value)}
              placeholder="Add task details..." rows={3}
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
            <div className="bg-white rounded-2xl p-4 shadow-sm">
              ...
              <DropDown
                open={showPriority}
                onToggle={() => { setShowPriority(!showPriority); setShowStatus(false); setShowAssignee(false); }}
                preview={
                  <div className="flex items-center gap-2">
                    <div style={{ width: 8, height: 8, borderRadius: "50%", background: pc.dot }} />
                    <span style={{ fontSize: 13, fontWeight: 600, color: pc.text }}>{priority}</span>
                  </div>
                }
              >
                {PRIORITIES.map((p, i) => {
            ...
            <div className="bg-white rounded-2xl p-4 shadow-sm">
              ...
              <DropDown
                open={showStatus}
                onToggle={() => { setShowStatus(!showStatus); setShowPriority(false); setShowAssignee(false); }}
                preview={
                  <div className="flex items-center gap-2">
                    <div style={{ width: 8, height: 8, borderRadius: "50%", background: statusInfo.dot }} />
                    <span style={{ fontSize: 13, fontWeight: 600, color: "#111827" }}>{statusInfo.label}</span>
                  </div>
                }
              >
                {STATUSES.map((s, i) => (
          ...
          <div className="bg-white rounded-2xl p-4 shadow-sm">
            <div className="flex items-center gap-1.5 mb-3">
              ...
              <span style={{ fontSize: 13, fontWeight: 600, color: "#374151" }}>Due Date</span>
            </div>
            ...
            {dateOpen && (
              <div style={{ marginTop: 8 }}>
                <DatePicker month={month} day={day} year={year}
                  onChange={(m, d, y) => { setMonth(m); setDay(d); setYear(y); }} />
                <button style={confirmBtn} onClick={() => { setDateSet(true); setDateOpen(false); }}>Confirm Date</button>
              </div>
            )}
          </div>
          ...
          <div className="bg-white rounded-2xl p-4 shadow-sm">
            <div className="flex items-center gap-1.5 mb-3">
              ...
              <span style={{ fontSize: 13, fontWeight: 600, color: "#374151" }}>Due Time</span>
            </div>
      ...
      <div className="bg-white flex-shrink-0 px-4 pt-2 pb-3" style={{ borderTop: "1px solid #F3F4F6" }}>
        ...
        <LoadingButton
          ...
          style={{ background: isValid ? "#2563EB" : "#E5E7EB" }}
        />
      </div>
    </PhoneFrame>
  );
